...
class Role extends Model
{
    ...
    static function get_role_menu($roleId){
        return DB::table('role_node')->where('role_id',$roleId)->pluck('node_code');
    }
    ...
    static function role_save($id=0,$roleName,$authCode=[]){
        ...
        try {
            if ($id) {    //更新roleName，并删除所有权限
                DB::table('role')->where('id', $id)->update(['role_name' => $roleName, 'update_time' => time()]);
                DB::table('role_node')->where('role_id',$id)->delete();
            } else {      //插入roleName
                $id = DB::table('role')->insertGetId(['role_name' => $roleName,'type'=>1,'create_time' => time()]);
            }
            if ($authCode) {
