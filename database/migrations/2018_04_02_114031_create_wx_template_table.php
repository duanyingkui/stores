...
class CreateWxTemplateTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('wx_template',function (Blueprint $table) {
            ...
            $table->text('template_rule')->comment('模板配置规则（json串）');// 模板配置规则（json串）[{key:显示名称,wx_key:微信模板提供,value:内容,type:0,color:'#000000',show:false,from:'占位符'}]
            ...
            $table->text('receipt_rule')->nullable()->comment('回执规则[{id:0,name:""}]');           
            // $table->comment('微信模板表');
        });
    }
